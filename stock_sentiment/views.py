...
def fetch_graph_data(request):
    ...
    list_of_all_dates = []
    without_neutral = all_comments.filter(Q(label='pos') | Q(label='neg')).all()
    ...
    end_date = time.mktime(datetime.datetime.strptime(unique_items[-1], "%Y-%m-%d").timetuple())
    print(inital_date,int(inital_date),type(inital_date),end_date,int(end_date),type(end_date))
    r = requests.get('https://finnhub.io/api/v1/stock/candle?symbol={0}&resolution=D&from={1}&to={2}&token=c0mrupn48v6tkq13hgo0'.format(ticker.ticker_name,
